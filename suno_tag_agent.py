...
class SunoTagAgent:
    ...
    def run(self):
        ...
        print(f"{self.C_GRAY}多行提交: Alt+Enter 或 Esc+Enter。{self.C_RESET}")
        self.resume_workflow()
        
        try:
            while True:
                if self.state == "INIT": self.handle_init()
                elif self.state == "INPUT_TITLE": self.handle_input_title()
                elif self.state == "INPUT_LYRICS": self.handle_input_lyrics()
                elif self.state == "TAGGING": self.handle_tagging()
                elif self.state == "STYLE_DISCUSSION": self.handle_style_discussion()
                elif self.state == "ENDING": self.handle_ending()
    ...
    def handle_input(self, prompt_text, multiline=False):
        ...
        while True:
            try:
                ...
                print(f"\n{self.C_YELLOW}[已取消]{self.C_RESET}")
                return "/quit"
            if not user_input: return ""
            first_line = user_input.split('\n')[0].lower().strip()
            if first_line == "/quit": sys.exit(0)
            if first_line == "/help": self.show_help(); continue
            if first_line == "/model":
                if not self.in_model_selection:
                    self.select_model()
                    if self.song_title: self.save_init()
                    continue
            if first_line in ["/m", "/multiline"]: return self.handle_input(prompt_text, multiline=True)
            if first_line == "/new": return "/new"
            return user_input
    ...
    def call_ollama(self, prompt, system_prompt="", stream=True, temperature=0.7):
        ...
        try:
            ...
            for line in resp.iter_lines():
                ...
                if reasoning:
                    if is_first_chunk:
                        print(f"{self.C_GRAY}[内部逻辑分析中...]{self.C_RESET}", end='', flush=True)
                        is_first_chunk = False
                    # 思考内容仍保持灰色但不作为主体显示
                    # 如果需要完全隐藏思考内容，可以注释掉下面这行
                    # print(f"{self.C_GRAY}{reasoning}{self.C_RESET}", end='', flush=True)
                    continue
    ...
    def check_intent(self, user_input, stage="内容确认"):
        ...
        chat_keywords = ['吗', '呢', '呗', '为什么', '如何', '怎么', '评价', '分析', '解释', '觉得', '理由']
    ...
    def clear_tmp_files(self):
        ...
        self.song_title = ""; self.safe_title = ""; self.version = 0; self.s_version = 0
    ...
    def select_model(self):
        ...
        except: print(f"{self.C_YELLOW}无法连接 Ollama。{self.C_RESET}"); return
        if not models: return
        print(f"\n{self.C_GRAY}模型列表:{self.C_RESET}")
        for i, m in enumerate(models):
            print(f"{i+1}. {m} {'(当前)' if m == self.model else ''}")
    ...
    def resume_workflow(self):
        if os.path.exists("tmp_tag_init.md"):
            ...
            if last_model:
                ...
                confirm = self.handle_input(f"是否继续使用 {self.C_YELLOW}{last_model}{self.C_RESET}? (y/n): ")
                if confirm.lower() in ['y', 'yes', '']: self.model = last_model
                else: self.select_model(); self.save_init()
        ...
        if self.safe_title:
            ...
            if style_files:
                ...
                with open(style_files[-1], 'r', encoding='utf-8') as f: self.style_suggestions = f.read()
            ...
            if tag_files:
                ...
                with open(tag_files[-1], 'r', encoding='utf-8') as f: self.tagged_lyrics = f.read()
                if self.state != "STYLE_DISCUSSION": self.state = "TAGGING"
                print(f"{self.C_GREEN}已成功恢复!{self.C_RESET}"); return
            else: self.state = "INPUT_LYRICS"; return
        self.state = "INIT"
    ...
    def handle_input_title(self):
        ...
        if os.path.exists(save_path):
            ...
            load_confirm = self.handle_input("是否加载并重做? (y/n): ")
            if load_confirm.lower() in ['y', 'yes', '']:
                with open(save_path, 'r', encoding='utf-8') as f: content = f.read()
                parts = content.split("\n\n=== Suggested Styles ===\n\n")
                self.tagged_lyrics = parts[0].strip(); self.style_suggestions = parts[1].strip() if len(parts)>1 else ""
                self.lyrics_original = re.sub(r'\[.*?\]\n?', '', self.tagged_lyrics)
                self.version = 1; self.s_version = 1 if self.style_suggestions else 0
                self.save_init()
                with open(f"tmp_tag_{self.safe_title}_v00.txt", 'w', encoding='utf-8') as f: f.write(f"{self.song_title}\n\n{self.lyrics_original}")
                with open(f"tmp_tag_{self.safe_title}_v01.txt", 'w', encoding='utf-8') as f: f.write(self.tagged_lyrics)
                if self.style_suggestions:
                    with open(f"tmp_tag_{self.safe_title}_styles_v01.txt", 'w', encoding='utf-8') as f: f.write(self.style_suggestions)
                print(f"{self.C_GREEN}已加载。{self.C_RESET}"); self.state = "TAGGING"; return
        self.save_init(); self.state = "INPUT_LYRICS"
    ...
    def handle_input_lyrics(self):
        ...
        self.lyrics_original = lyrics_input; self.version = 0
        with open(f"tmp_tag_{self.safe_title}_v00.txt", 'w', encoding='utf-8') as f: f.write(f"{self.song_title}\n\n{lyrics_input}")
        self.state = "TAGGING"

    def handle_tagging(self):
        self.show_header("歌词标注")
        system_prompt = "你是一位专业的 SUNO AI 编曲专家。为歌词添加专业的 Meta Tags。严禁括号内含中文。段落前加标注。保持歌词文字。标题开头。不要解释。"
        just_streamed = False
        if self.version == 0:
            full_text = f"{self.song_title}\n\n{self.lyrics_original}"
            self.tagged_lyrics = self.call_ollama(f"专业标注：\n\n{full_text}", system_prompt=system_prompt)
        ...
        if not just_streamed:
            ...
            print(self.tagged_lyrics); print(f"---{'-'*20}---")
        suggestion = self.handle_input("\n建议/问题 (ok 批准并进入风格, vX 回退, /c 仅讨论):")
        ...
        if back_match:
            v = int(back_match.group(1)); back_file = f"tmp_tag_{self.safe_title}_v{v:02d}.txt"
            if os.path.exists(back_file) and "_styles_" not in back_file:
                self.version = v; self.tagged_lyrics = open(back_file,'r',encoding='utf-8').read(); print(f"已回退 v{v:02d}"); return
        intent, clean_input = self.check_intent(suggestion, "标注阶段")
        if intent == "APPROVE": self.state = "STYLE_DISCUSSION"; self.s_version = 0; return
        elif intent == "CHAT":
            self.call_ollama(f"咨询《{self.song_title}》标注：'{clean_input}'\n内容：\n{self.tagged_lyrics}", 
                             system_prompt="你是编曲家。直接回答用户建议，不要输出多方案，不要重写歌词。")
        elif intent == "MODIFY":
            prompt = f"修改《{self.song_title}》标注：'{clean_input}'\n当前：\n{self.tagged_lyrics}\n直接输出修改后的完整标注歌词。"
            new_tagged = self.call_ollama(prompt, system_prompt=system_prompt)
            if new_tagged:
                self.tagged_lyrics = new_tagged; self.version += 1
                with open(f"tmp_tag_{self.safe_title}_v{self.version:02d}.txt", 'w', encoding='utf-8') as f: f.write(self.tagged_lyrics)
    ...
    def handle_style_discussion(self):
        ...
        just_streamed = False
        if self.s_version == 0:
            self.style_suggestions = self.call_ollama(f"策划风格：\n\n{self.tagged_lyrics}", system_prompt=system_prompt)
            if self.style_suggestions:
                self.s_version = 1; with open(f"tmp_tag_{self.safe_title}_styles_v{self.s_version:02d}.txt", 'w', encoding='utf-8') as f: f.write(self.style_suggestions)
                just_streamed = True
            else: return
        if not just_streamed:
            print(f"\n--- {self.C_CYAN}当前风格 (sv{self.s_version:02d}){self.C_RESET} ---")
            print(self.style_suggestions); print(f"---{'-'*20}---")
        suggestion = self.handle_input("\n建议/问题 (ok 批准并保存, svX 回退, /c 仅讨论):")
        if suggestion == "/new": self.clear_tmp_files(); self.state = "INIT"; return
        if suggestion == "/quit": return
        back_match = re.search(r"back to sv(\d+)", suggestion.lower()) or re.search(r"^sv(\d+)$", suggestion.lower())
        if back_match:
            v = int(back_match.group(1)); back_file = f"tmp_tag_{self.safe_title}_styles_v{v:02d}.txt"
            if os.path.exists(back_file):
                self.s_version = v; self.style_suggestions = open(back_file,'r',encoding='utf-8').read(); print(f"已回退 sv{v:02d}"); return
        ...
        if intent == "APPROVE":
            ...
            print(f"\n{self.C_GREEN}已保存到: {save_path}{self.C_RESET}"); self.clear_tmp_files(); self.state = "ENDING"; return
        elif intent == "CHAT":
            self.call_ollama(f"咨询《{self.song_title}》风格：'{clean_input}'\n当前：\n{self.style_suggestions}", 
                             system_prompt="你是风格策划师。直接回答用户建议，不要输出多方案，不要输出列表。")
        elif intent == "MODIFY":
            new_styles = self.call_ollama(f"修改《{self.song_title}》风格：'{clean_input}'\n当前：\n{self.style_suggestions}", system_prompt=system_prompt)
            if new_styles:
                self.style_suggestions = new_styles; self.s_version += 1
                with open(f"tmp_tag_{self.safe_title}_styles_v{self.s_version:02d}.txt", 'w', encoding='utf-8') as f: f.write(self.style_suggestions)

    def handle_ending(self):
        choice = self.handle_input("\n是否处理其他歌曲? (yes/no):")
        if choice.lower() in ['yes', 'y', '好的', '是']: self.state = "INIT"
        else: print("再见！"); sys.exit(0)

if __name__ == "__main__":
    agent = SunoTagAgent(); agent.run()
